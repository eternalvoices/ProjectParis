...
class PP_Event {
	...
	function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'created_by'  => $bp->loggedin_user->id,
			'last_updated_by'  => $bp->loggedin_user->id,
			'participants_id_list' => array(),
		);
	...
	function populate() {
		...
		if ( $row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM pp_event WHERE id = %d", $this->id ) ) ) {
			$this->id					= $row->id;
	...
	function save() {
		...
		do_action( 'bp_event_data_before_save', $this );

		if ( $this->id ) {
			// Update the event

		} else {
			// Insert a new event to pp_event
			$wpdb->insert( 
					'pp_event', 
					array( 
			...
			// Retrieve the id of inserted event
			$this->id = $wpdb->insert_id;
			// Insert participants to pp_event_user
			foreach ($this->participants_id_list as $participant){		
				$wpdb->insert( 
					'pp_event_user', 
					array( 
						'event_id' => $this->id,
