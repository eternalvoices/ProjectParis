...
function pp_add_event_action() {
	
	if ( bp_is_example_component() && bp_is_current_action( 'pp-add-event' ) && bp_is_action_variable( 'add-event', 0 ) ) {
		pp_add_event_function();

		bp_core_redirect( bp_displayed_user_domain() . bp_get_example_slug() . '/pp-add-event' );
	}
}
add_action( 'bp_actions', 'pp_add_event_action');
