...
function pp_add_event_function( ) {
	global $bp;

	//check_admin_referer( 'pp_add_event_function' );
	
	// Read invitee list after deleting the first '@'
	$invitee_name_list=explode('@', substr($_POST['pp-event-add-invitee-list'],1));
	
	$inventee_id_list = array();

	foreach ($invitee_name_list as $invitee_name){
		if (bp_core_get_userid($invitee_name)){
			array_push ($inventee_id_list, bp_core_get_userid($invitee_name));
		}else {
	...
	$participants_id_list=$inventee_id_list;
	
	// Add inviter to the participants list
	array_push($participants_id_list, $bp->loggedin_user->id);
	
	$db_args = array (
		'event_name'			=> $_POST['pp-event-add-event-name'],
		'event_description'		=> $_POST['pp-event-add-event-description'],
		'place_id'				=> $_POST['pp-event-add-place-id'],
		'event_type'			=> $_POST['pp-event-add-event-type'],
		'event_time'			=> $_POST['pp-event-add-event-time'],
		'participants_id_list'	=> $participants_id_list,
	);
	
	// Initialise event
	$new_pp_event = new PP_Event( $db_args );
	
	//bp_core_add_message ($new_pp_event->participants_id_list[2]);
	
	// Save event
	$new_pp_event->save();
	
	// Send message
	foreach ($inventee_id_list as $invitee_id){
		if ( $thread_id = messages_new_message( array( 'recipients' => $inventee_id_list, 'subject' => 'Invite you to event'.$new_pp_event->event_name, 'content' => 'Please join me' ) ) ) {
			bp_core_add_message( __( 'Event saved successfully', 'buddypress' ) );
		} else {
			bp_core_add_message( __( 'There was an error sending that message, please try again', 'buddypress' ), 'error' );
			return 0;
		}
